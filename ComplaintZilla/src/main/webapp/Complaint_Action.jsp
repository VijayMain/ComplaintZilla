...
<body>
		...
		<div class="page-full-width cf">
			...
			<ul id="nav" class="fl">
				...
				<li><a href="logout.jsp"
					class="round button dark menu-logoff image-left">Log out <b>(<%=dept_name%>)
					</b></a></li>
			</ul>
			<!-- end nav -->
	...
	<div id="header-with-tabs">
		...
		<div class="page-full-width cf">
			...
			<ul id="tabs" class="fl">
				...
				<li><a href="Edit_By_Search_Other.jsp"
					class="active-tab dashboard-tab">Search</a></li>
			...
			<div class="side-content fr">
				...
				<div class="content-module">
					...
					<div id="form_container" style="float: left; width: 90%">
						...
						<form class="appnitro" action="Complaint_Action_Controller"
							name="myForm" id="myForm" method="post"
							enctype="multipart/form-data"
							onSubmit="return validateComplaint();">

							<%
								String comp_type = "";
									PreparedStatement ps = con.prepareStatement("select * from customer_tbl");
									PreparedStatement ps1 = con.prepareStatement("select * from customer_tbl_item");
									PreparedStatement ps11 = con.prepareStatement("select * from status_tbl");
									PreparedStatement ps2 = con.prepareStatement("select * from defect_tbl");
									PreparedStatement ps3 = con.prepareStatement("select * from complaint_tbl");
									PreparedStatement ps4 = con.prepareStatement("select * from category_tbl");
									...
									String cust_name = null, item_name = null, defect = null, category = null, cust_comp = null,
											unit_name = null;
									int max_Action_Id = 0;

									PreparedStatement ps_MaxActionId = null;

									ps_MaxActionId = con.prepareStatement(
											"select max(Action_id) from complaint_tbl_action where Complaint_No='" + complaint_no + "'");
									...
									String dept = null;
									PreparedStatement ps_user_dept = con.prepareStatement(
											"select department from user_tbl_dept where dept_Id=(select dept_id from user_tbl where u_id="
													+ uid + ")");
									...
									while (rs5.next()) {
										...
										PreparedStatement ps7 = con
												.prepareStatement("select Cust_Name from customer_tbl where Cust_Id=" + cust_Id);
										PreparedStatement ps8 = con
												.prepareStatement("select Item_Name from customer_tbl_item where Item_Id=" + Item_Id);
										PreparedStatement ps9 = con
												.prepareStatement("select Defect_Type from defect_tbl where Defect_Id=" + Defect_Id);
										PreparedStatement ps10 = con
												.prepareStatement("select Category from category_tbl where Category_Id=" + Category_Id);
										PreparedStatement ps_act = con
												.prepareStatement("select * from user_tbl_company where Company_Id='" + comp_No + "'");
									...
									PreparedStatement ps_u_name = con
											.prepareStatement("Select U_Name,Dept_Id,Company_Id from user_tbl where u_id=" + uid);
									...
									PreparedStatement ps_u_id = con
											.prepareStatement("Select U_Id from user_tbl where U_Name='" + u_name + "'");
							...
							<ul>
								...
								<li id="li_1"><label class="description" for="element_1">Action
									...
									<div>
										...
										<table style="font-size: small; width: 700px;">
											...
											<%
												PreparedStatement PS_AP = con.prepareStatement(
																"select * from complaint_tbl_action where Complaint_No='" + complaint_no + "'");
								...
								<li id="li_8"><label class="description" for="element_8">CHOOSE
										PHASE </label> <%
 	PreparedStatement ps_selectedPhase = con.prepareStatement(
 					"select phase_id from complaint_tbl_action where Action_Id=" + max_Action_Id);
 			int phase_id = 0;
 			ResultSet rs_selectedPhase = ps_selectedPhase.executeQuery();
 ...
 %>
								...
								<li id="li_4" class="buttons"><label class="description"
									...
									<div>
										<div>
												...
												<%
													PreparedStatement ps_file = con
																	.prepareStatement("select * from complaint_tbl_attachments where Complaint_No like'"
																			+ complaint_no + "'" + "and  delete_Status=1");
															ResultSet rs_file = ps_file.executeQuery();
															while (rs_file.next()) {
																//System.out.println("Files Attached " + rs_file.getString("File_Name"));
																//System.out.println("Primary key = " + rs_file.getInt("Attach_Id"));
												%>
												<tr>
													<td><a
											...
											<table>
												<%
													PreparedStatement ps_un = con.prepareStatement(
																	"select * from complaint_unassigned_rel_tbl where Complaint_No='" + complaint_no + "'");
															...
															if (attch > 0) {
																while (rs_un.next()) {
																	PreparedStatement ps_unassign = con.prepareStatement(
																			"select * from complaint_tbl_unassigned_attachment where unassign_Id="
												...
												<tr>
													<td><a
														href="ViewCustAttachment.jsp?field=<%=rs_unassign.getInt("Unasigned_Attach_id")%>"><%=rs_unassign.getString("File_Name")%></a>
													</td>
												</tr>
												<%
													}
								...
								<li><a href="Home.jsp"><strong
										style="font-size: large;">&#8656;Back</strong></a> <input
									type="submit" name="submit" id="form_submit"
									style="width: 200px; height: 40px; font-size: 15px; font-family: Arial; background-color: #9191C2"
									value="SUBMIT" /> <br> <span id="waitNote"
									style="visibility: hidden;"><strong style="color: blue;">Please
											Wait while loading......</strong></span>
							</ul>
							<%
								} else {
							%>
							<ul>
								...
								<li id="li_1"><label class="description" for="element_1">COMPLAINT
										DESCRIPTION </label>
									<div>
										<textarea id="element_1" name="description" cols="50" rows="5"
											class="element textarea medium" readonly="readonly"><%=discription%></textarea>
 ...
 %>
								...
								<li><a href="Home.jsp"><strong
									...
									value="SUBMIT" /><br> <span id="waitNote"
									style="visibility: hidden;"><strong style="color: blue;">Please
											Wait while loading......</strong></span></li>
								<%
									} else {
								%>
								<li><a href="Home.jsp"><strong
					...
					<div style="float: right; width: 40%; background-color: #DBDBDB">
						<%
							int unassignId = 0;
								PreparedStatement ps_reply = con.prepareStatement(
										"select * from complaint_unassigned_rel_tbl where complaint_no='" + complaint_no + "'");
						...
						%>
						<br> <span><strong
							style="font-size: 16px; color: #727502">Customers Reply
								History:</strong></span>
