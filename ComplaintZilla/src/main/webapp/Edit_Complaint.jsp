	...
	<div id="header-with-tabs">
		...
		<div class="page-full-width cf">
			...
			<ul id="tabs" class="fl">
				...
				<li><a href="Edit_By_Search.jsp"
					class="active-tab dashboard-tab">Search</a></li>
				<li><a href="Dashboard_mkt.jsp"
					class="active-tab dashboard-tab">Dashboard</a></li>
			</ul>
			<!-- end tabs -->
						...
						<form class="appnitro" action="Edit_Complaint_Controller"
							...
							<%
								/****************************************************************************************************************
									...
									****************************************************************************************************************/
									...
									PreparedStatement ps13 = con.prepareStatement("select * from user_tbl_company");
									...
									String cust_name = null, company_name = null, unit_name = null, company_name1 = null, item_name = null,
											defect = null, category = null;
									...
									while (rs5.next()) {
										...
										PreparedStatement ps_ass = con
												.prepareStatement("select U_Name from User_Tbl where U_Id=" + assigned_id);
										...
										PreparedStatement ps_rel = con
												.prepareStatement("select Department from User_Tbl_dept where dept_Id=" + related_id);
										...
										PreparedStatement ps12 = con.prepareStatement(
												"select company_name from user_tbl_company where company_id=" + company_id);
									...
									PreparedStatement ps_p = con.prepareStatement("select * from severity_tbl where p_id=" + p_id);
							...
							<ul>
											...
											<%
												//SELECT COMPONEMT NAMES
														PreparedStatement ps_item = con
																.prepareStatement("select item_id from customer_item_tbl where cust_id=" + cust_id);
														...
														for (int i_id = 1; i_id < al.size(); i_id++) {
															PreparedStatement ps_i = con.prepareStatement(
																	"select distinct Item_Name,Item_Id from customer_tbl_item where Item_id="
																			+ al.get(i_id));
								...
								<li id="li_1"><label class="description" for="element_1">Severity</label>
									<div>
										<select class="element select medium" id="element_6"
											...
											<%
												//SELECT SEVERITY
														PreparedStatement ps_pr = con.prepareStatement("select * from severity_tbl where P_Id!=" + p_id);
										...
										<select class="element select medium" id="element_10"
											...
											<%
												PreparedStatement ps4 = con
																.prepareStatement("select * from category_tbl where Category_Id!=" + Category_Id);
											...
											<%
												//SELECT COMPLAINT RELATED TO ( GET DATA FROM USER TABLE )
														PreparedStatement ps_rel1 = con
																.prepareStatement("select * from User_Tbl_dept where Dept_Id!=" + related_id);
										...
										<select class="element select medium" id="element_9"
											...
											<%
												// GET DATA FROM USER TABLE FOR COMPLAINT ASSIGN TO
														PreparedStatement ps17 = con.prepareStatement(
																"select U_Id,U_Name from user_tbl where enable_id=1 && U_Id!=" + assigned_id);
								...
								<li id="li_3"><label class="description" for="element_3">COMPLAINT
										Date </label> <input id="edit_date" name="edit_date" type="text"
									size="25" value="<%=str1%>" TITLE="Click on Date Picker"
