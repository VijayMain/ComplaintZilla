...
public class Login_DAO {
	...
	public void authenticate_Login(Login_VO bean, HttpSession session, HttpServletResponse response) {
		...
		try {
			...
			while (rs_user.next()) {
				...
				if (rs_user.getString("Login_Name").equalsIgnoreCase(bean.getLogin_Name())
						&& rs_user.getString("Login_Password").equalsIgnoreCase(bean.getLogin_Password())) {
					...
					PreparedStatement ps_swAccess = con
							.prepareStatement("select * from it_software_access_tbl where U_Id=" + uid);
					ResultSet rs_swAccess = ps_swAccess.executeQuery();

					if (rs_swAccess.equals("")) {
						str = "Access IS Denied . . . !";
						// System.out.println("software access tbl is empty .....");

					} else {

						while (rs_swAccess.next()) {
							// System.out.println("checking software access .....");
