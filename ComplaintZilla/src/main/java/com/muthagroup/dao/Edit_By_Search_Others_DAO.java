...
public class Edit_By_Search_Others_DAO {
	...
	public void editBySearch(Edit_By_Search_Others_VO bean, HttpServletRequest request, ServletContext sc,
		...
		try {
			...
			String complaint_no = null, received = null, discription = null, related = null, assigned = null,
					cdate = null;
			...
			if (bean.getComplaint_no() == "") {
				...
				if (date1 != null && date2 != null && status_id != 0 && cust_id != 0 && company_id != 0
						&& severity != 0)

				{

					ps = con.prepareStatement("select * from complaint_tbl where complaint_date between '" + date1
							+ "' and '" + date2 + "' and status_id=" + status_id + " and company_id=" + company_id
							+ " and P_Id=" + severity);
					rs = ps.executeQuery();
				}
				...
				else if (date1 != null && date2 != null && status_id == 0 && cust_id == 0 && company_id != 0
						&& severity == 0) {
					ps = con.prepareStatement("select * from complaint_tbl where complaint_date between '" + date1
							+ "' and '" + date2 + "' and Company_Id=" + company_id);
					rs = ps.executeQuery();
				}
				...
				// *****************************************************************************
						...
						&& severity == 0) {
					ps = con.prepareStatement("select * from complaint_tbl where complaint_date between '" + date1
							+ "' and '" + date2 + "' and status_id=" + status_id);
					rs = ps.executeQuery();
				}
				...
				else if (date1 == null && date2 == null && company_id != 0 && status_id == 0 && cust_id == 0
						&& severity == 0) {
					ps = con.prepareStatement("select * from complaint_tbl where company_id=" + company_id);
					rs = ps.executeQuery();
				}
				...
				// ******************************************************************1***********
						...
						&& severity == 0) {
					ps = con.prepareStatement("select * from complaint_tbl where status_id=" + status_id);
					rs = ps.executeQuery();
				}
				...
				// *****************************************************************************
						...
						&& severity == 0) {
					ps = con.prepareStatement("select * from complaint_tbl where Complaint_Date='" + date2 + "'");
					rs = ps.executeQuery();
				}
				...
				else if (date1 != null && date2 == null && status_id == 0 && cust_id == 0 && severity == 0) {
					ps = con.prepareStatement("select * from complaint_tbl where Complaint_Date='" + date1 + "'");
					rs = ps.executeQuery();
				}
				...
				// *****************************************************************************
					...
					ps = con.prepareStatement("select * from complaint_tbl where Complaint_No like '" + cust + "%'");
					rs = ps.executeQuery();
				}
				...
				// *****************************************************************************
						...
						&& severity == 0) {
					ps = con.prepareStatement("select * from complaint_tbl where Complaint_Date between '" + date1
							+ "' and '" + date2 + "' and Complaint_No like '" + cust + "%'");
					rs = ps.executeQuery();
				}
				...
				// *****************************************************************************
					...
					ps = con.prepareStatement(
							"select * from complaint_tbl where Complaint_Date between '" + date1 + "' and '" + date2
									+ "' and status_Id=" + status_id + " and Complaint_No like '" + cust + "%'");
					rs = ps.executeQuery();
				}
				...
				// *****************************************************************************
					...
					ps = con.prepareStatement(
							"select * from complaint_tbl where Complaint_Date between '" + date1 + "' and '" + date2
									+ "' and Company_Id=" + company_id + " and Complaint_No like '" + cust + "%'");
					rs = ps.executeQuery();
				}
				...
				// *****************************************************************************
						...
						&& severity == 0) {
					ps = con.prepareStatement("select * from complaint_tbl where status_Id =" + status_id
							+ " and Complaint_Date='" + date1 + "' and Complaint_No like '" + cust + "%'");
					rs = ps.executeQuery();
				}
				...
				// *****************************************************************************
						...
						&& severity == 0) {
					ps = con.prepareStatement("select * from complaint_tbl where status_Id =" + status_id
							+ " and Complaint_Date='" + date2 + "' and Complaint_No like '" + cust + "%'");
					rs = ps.executeQuery();
				}
				...
				// *****************************************************************************
						...
						&& severity == 0) {
					ps = con.prepareStatement("select * from complaint_tbl where Company_Id =" + company_id
							+ " and Complaint_Date='" + date2 + "' and status_Id=" + status_id
							+ " and Complaint_No like '" + cust + "%'");
					rs = ps.executeQuery();
				}
				...
				// *****************************************************************************
						...
						&& severity == 0) {
					ps = con.prepareStatement("select * from complaint_tbl where Company_Id =" + company_id
							+ " and Complaint_Date='" + date1 + "' and status_Id=" + status_id
							+ " and Complaint_No like '" + cust + "%'");
					rs = ps.executeQuery();
				}
				...
				// *****************************************************************************
						...
						&& severity == 0) {
					ps = con.prepareStatement("select * from complaint_tbl where Company_Id =" + company_id
							+ " and status_Id=" + status_id + " and Complaint_Date='" + date1 + "'");
					rs = ps.executeQuery();
				}
				...
				// *****************************************************************************
						...
						&& severity == 0) {
					ps = con.prepareStatement("select * from complaint_tbl where Company_Id =" + company_id
							+ " and status_Id=" + status_id + " and Complaint_Date='" + date2 + "'");
					rs = ps.executeQuery();
				}
				...
				// *****************************************************************************
						...
						&& severity == 0) {
					ps = con.prepareStatement("select * from complaint_tbl where Complaint_Date between '" + date1
							+ "' and '" + date2 + "' and Company_Id=" + company_id + " and status_Id=" + status_id);
					rs = ps.executeQuery();
				}
				...
				// *****************************************************************************
						...
						&& severity != 0) {
					ps = con.prepareStatement("select * from complaint_tbl where p_id=" + severity);
					rs = ps.executeQuery();
				}
				...
				// *****************************************************************************
						...
						&& severity != 0) {
					ps = con.prepareStatement(
							"select * from complaint_tbl where Company_Id=" + company_id + " and P_Id=" + severity);
					rs = ps.executeQuery();
				}
				...
				// *****************************************************************************
						...
						&& severity != 0) {
					ps = con.prepareStatement(
							"select * from complaint_tbl where Status_Id=" + status_id + " and P_Id=" + severity);
					rs = ps.executeQuery();
				}
				...
				// *****************************************************************************
						...
						&& severity != 0) {
					ps = con.prepareStatement(
							"select * from complaint_tbl where Complaint_no like '" + cust + "%' and P_Id=" + severity);
					rs = ps.executeQuery();
				}
				...
				// *****************************************************************************
						...
						&& severity != 0) {
					ps = con.prepareStatement(
							"select * from complaint_tbl where Complaint_Date='" + date1 + "' and P_Id=" + severity);
					rs = ps.executeQuery();
				}
				...
				// *****************************************************************************
						...
						&& severity != 0) {
					ps = con.prepareStatement(
							"select * from complaint_tbl where Complaint_Date='" + date2 + "' and P_Id= " + severity);
					rs = ps.executeQuery();
				}
				...
				// *****************************************************************************
						...
						&& severity != 0) {
					ps = con.prepareStatement("select * from complaint_tbl where Complaint_Date between '" + date1
							+ "' and '" + date2 + "' and P_Id=" + severity + " and Complaint_no like '" + cust + "%'");
					rs = ps.executeQuery();
				}
				...
				// *****************************************************************************
						...
						&& severity != 0) {
					ps = con.prepareStatement("select * from complaint_tbl where Complaint_Date between '" + date1
							+ "' and '" + date2 + "' and P_Id=" + severity + " and status_id=" + status_id);
					rs = ps.executeQuery();
				}
				...
				// *****************************************************************************
						...
						&& severity != 0) {
					ps = con.prepareStatement("select * from complaint_tbl where Complaint_Date between '" + date1
							+ "' and '" + date2 + "' and P_Id=" + severity + " and company_id=" + company_id);
					rs = ps.executeQuery();
				}
				...
				// *****************************************************************************
					...
					ps = con.prepareStatement(
							"select * from complaint_tbl where Complaint_Date ='" + date1 + "' and P_Id=" + severity
									+ " and company_id=" + company_id + " and status_id=" + status_id);
					rs = ps.executeQuery();
				}
				...
				else if (cust_id != 0 && severity != 0 && status_id != 0 && date1 == null && date2 == null
						&& company_id == 0) {
					ps = con.prepareStatement("select * from complaint_tbl where Cust_Id=" + cust_id + " and P_Id="
				...
				// ************************************************************************************
						...
						&& company_id == 0) {
					ps = con.prepareStatement(
							"select * from complaint_tbl where P_Id=" + severity + " and Status_Id=" + status_id);
					rs = ps.executeQuery();
				}
			} else if (bean.getComplaint_no() != null) {
				ps = con.prepareStatement(
						"select * from complaint_tbl where Complaint_no='" + bean.getComplaint_no() + "'");
				rs = ps.executeQuery();
			} else {
			...
			RequestDispatcher rd = request.getRequestDispatcher("/Search_Result_Others.jsp");
