...
public class Complaint_Action_Dao {
	...
	String Complaint_Received_By = null, Complaint_Description = null, custName = null, company = null, part = null;
	int cust_id = 0, item_id = 0, Defect_Id = 0, Complaint_Related_To = 0, Complaint_Assigned_To = 0, Category_Id = 0,
			Company_Id = 0, P_Id = 0, status_id = 0;
	int count = 0;

	// System.out.println("by TIMESTAMP..:" + timestamp);

	public boolean complaint_action(Complaint_Action_VO bean, HttpSession session) {
		boolean flag = false;
		try {
			...
			String complaint_no = session.getAttribute("complaint_no").toString();
			...
			ResultSet rs_atype = st.executeQuery("select * from Action_Type_Tbl");
			...
			if (flag1 == false) {
				PreparedStatement st1 = con.prepareStatement("insert into Action_Type_Tbl (Action_Type)values(?)");
			...
			PreparedStatement ps = con
					.prepareStatement("select * from complaint_tbl where Complaint_No='" + complaint_no + "'");
			...
			PreparedStatement ps_stat_up = con
					.prepareStatement("update complaint_tbl set Status_Id=? where Complaint_No='" + complaint_no + "'");
			...
			if (bean.getStatus() != status_id) {
				...
				PreparedStatement ps_status = con
						.prepareStatement("select * from status_tbl where Status_Id=" + bean.getStatus());
				...
				PreparedStatement ps_old_stat = con
						.prepareStatement("select * from status_tbl where Status_Id=" + status_id);
				...
				PreparedStatement ps_user = con.prepareStatement("select * from user_tbl where U_Id=" + uid);
				...
				PreparedStatement ps_cust = con.prepareStatement("select * from customer_tbl where Cust_Id=" + cust_id);
				...
				PreparedStatement ps_comp = con
						.prepareStatement("select * from user_tbl_company where Company_Id=" + Company_Id);
				...
				PreparedStatement ps_part = con
						.prepareStatement("select * from customer_tbl_item where Item_Id=" + item_id);
				...
				String subject = company + " " + part + " Complaint Status is " + status_change;
				// String messageText =
				...
				PreparedStatement ps6 = con.prepareStatement(
						"select count(Email) from automail where company_id=6 or company_id=" + Company_Id);
				...
				PreparedStatement ps_auto = con
						.prepareStatement("select * from automail where company_id=6 or company_id=" + Company_Id);
				...
				msg.setContent("<b>Complaint Number : </b>" + complaint_no + ", "
						+ "<p><b>Status changed from : </b>" + old_status + "<b> to </b>" + status_change + ", "
						+ " </p>"

						+ "<p><b>Action Type : </b>" + bean.getAction_Type() + ", " + " </p>"
						+ "<p><b>Complaint Action Description :</b></p><p>" + bean.getAction_description() + "</p>"

						+ "<p><b>Action Taken By : </b>" + user1 + ", " + " </p>"

						+ "<p><b>Complaint Received date :</b>" + Complaint_Date + ", " + " </p>"
						...
						+ "<p><b style='color: #330B73;'>Thanks & Regards </b></br> Mutha Group Satara </p>" + "<hr>"
						+ "<p><b>Disclaimer :</b></p>" + "<font face='Times New Roman' size='1'>"
				 ...
				 * JasperCompileManager.compileReport("C:/JRXMLFILES/CZReport.jrxml"); Map
				...
				/*
				 * String filename = "C:\\ComplaintZilla\\Report.pdf"; DataSource source = new
			...
			PreparedStatement ps_action = con.prepareStatement(
					"insert into complaint_tbl_action(Complaint_No,U_Id,Action_Discription,Action_Date,Action_Type,phase_id)values(?,?,?,?,?,?)");
			...
			PreparedStatement ps_a_id = con.prepareStatement("select max(Action_Id) from Complaint_Tbl_Action");
			...
			if (i > 0) {
				PreparedStatement ps_action_hist = con.prepareStatement(
						"insert into complaint_tbl_action_hist(Action_Id,Complaint_No,U_Id,Action_Discription,Action_Date,Action_Type,Action_Date_Hist,Phase_Id)values(?,?,?,?,?,?,?,?)");
			...
			PreparedStatement ps_acthistory = con.prepareStatement(
					"insert into complaint_tbl_history(C_History_Date,Complaint_No,U_Id,Cust_Id,Item_Id,Defect_Id,Complaint_Received_By,Complaint_Description,Complaint_Related_To,Complaint_Assigned_To,Category_Id,Status_Id,Company_Id,Complaint_Date,P_Id)values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
	...
	public boolean file_upload(Complaint_Action_VO bean, HttpSession session) {
		...
		try {
			...
			// ***********************************************************************************
			// Attach file
			PreparedStatement ps_file = con.prepareStatement(
					"insert into complaint_tbl_attachments(Complaint_No,Attachment,Attach_Date,U_Id,File_Name,delete_Status)values(?,?,?,?,?,?)");
			...
			PreparedStatement ps_attach_history = con.prepareStatement(
					"insert into complaint_tbl_attachment_hist(CA_Hist_date,Complaint_No,Attach_Date,U_Id,File_Name,Delete_Status_Hist)values(?,?,?,?,?,?)");
			...
			if (bean.getFile_Name() != "") {
				...
				try {
					...
					while (rs_email.next()) {
						...
						PreparedStatement ps_ccmp = con.prepareStatement(
								"select * from user_tbl_company where Company_Id=" + rs_email.getInt("Company_Id"));
						ResultSet rs_cmp = ps_ccmp.executeQuery();
						while (rs_cmp.next()) {
							company = rs_cmp.getString("Company_Name");
						}
						PreparedStatement ps_part = con.prepareStatement(
								"select * from customer_tbl_item where Item_Id=" + rs_email.getInt("Item_Id"));
						ResultSet rs_part = ps_part.executeQuery();
						while (rs_part.next()) {
							part = rs_part.getString("Item_Name");
						}
					...
					String from = "complaintzilla@muthagroup.com";
					String subject = company + " " + part + " " + "New File attached, Complaint No : "
					...
					// *********************************************************************************************
					PreparedStatement ps6 = con.prepareStatement(
							"select count(Email) from automail where company_id=" + company_id + " or company_id=6");
					ResultSet rs6 = ps6.executeQuery();
					while (rs6.next()) {
						count = rs6.getInt("count(Email)");
					}
					// System.out.println("Company_Id ... : " + company_id);
					PreparedStatement ps_auto = con.prepareStatement(
							"select * from automail where company_id=" + Company_Id + " or company_id=6");
					ResultSet rs_auto = ps_auto.executeQuery();
					...
					try {
						...
						// Fill the message
						// messageBodyPart.setText("This is message body");
						messageBodyPart.setContent("Please find the newly attached File to Complaint No : "
								+ bean.getComplaint_no() + "</b>"
								+ "<p><font size='2'><b style='color: #0D265E;'>This is an automatically generated email from ComplaintZilla</b></font>"
								+ "</p>" + "<p><b>For more details ,</b> "
								+ "<a href='http://192.168.0.7/ComplaintZilla/'>Click Here</a></p>"
						...
						try {
							...
							PreparedStatement ps_mail = con.prepareStatement(
									"select * from complaint_tbl_attachments where File_Name='" + bean.getFile_Name()
											+ "' and Complaint_No='" + bean.getComplaint_no() + "'");
							...
							while (rs_mail.next()) {
								...
								FileOutputStream fos = new FileOutputStream(
										"c:\\EmailFiles" + "\\" + rs_mail.getString("File_Name"));
								int b = 0;
								while ((b = is.read()) != -1) {
									fos.write(b);
								}
								 ...
								 * .compileReport("C:/JRXMLFILES/CZReport.jrxml" ); Map parameters = new
								 * HashMap(); JasperPrint jasperPrint = JasperFillManager.fillReport(
								 * jasperReport, parameters, con); JasperExportManager.exportReportToPdfFile
		...
		try {
			...
			if (bean.getStatus() != status_id) {
				...
				PreparedStatement ps_defect = con
						.prepareStatement("select * from defect_tbl where Defect_Id=" + Defect_Id);
				...
				String from = "complaintzilla@muthagroup.com";
				String subject = company + " " + item + " Complaint Status is " + status_change;
				...
				// Fill the message
				// messageBodyPart.setText("This is message body");
				msg.setContent("<p><b>Complaint Number :</b>" + complaint_no + ", "
						...
						+ "</p>" + "<p><b>For more details ,</b> "
						+ "<a href='http://192.168.0.7/ComplaintZilla/'>Click Here</a></p>"
						+ " <p><b style='color: #330B73;'>Thanks & Regards </b></p><p> Mutha Group Satara </p>" + "<hr>"
						+ "<p><b>Disclaimer :</b></p>" + "<font face='Times New Roman' size='1'>"
						+ "<p><b style='color: #49454F;'>The information transmitted, including attachments, is intended only for the person(s) or entity to which it is addressed and may contain confidential and/or privileged material. Any review, retransmission, dissemination or other use of, or taking of any action in reliance upon this information by persons or entities other than the intended recipient is prohibited. If you received this in error, please contact the sender and destroy any copies of this information.</b></p></font>",
						"Text/html");
				/*// Create a multipar message
			...
			if (i > 0) {
				...
				while (rs_getDetails.next()) {
					System.out
							.println("Complaint No when changing status : " + rs_getDetails.getString("Complaint_No"));

					ps_addDetails = con.prepareStatement(
							"insert into complaint_tbl_history(C_History_Date,Complaint_No,U_Id,Cust_Id,Item_Id,Defect_Id,Complaint_Received_By,Complaint_Description,Complaint_Related_To,Complaint_Assigned_To,Category_Id,Status_Id,Company_Id,Complaint_Date,P_Id)values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
					...
					ps_addDetails.setString(7, rs_getDetails.getString("Complaint_Received_By"));
					ps_addDetails.setString(8, rs_getDetails.getString("Complaint_Description"));
					ps_addDetails.setInt(9, rs_getDetails.getInt("Complaint_Related_To"));
					ps_addDetails.setInt(10, rs_getDetails.getInt("Complaint_Assigned_To"));
					ps_addDetails.setInt(11, rs_getDetails.getInt("Category_Id"));
					ps_addDetails.setInt(12, rs_getDetails.getInt("Status_Id"));
					ps_addDetails.setInt(13, rs_getDetails.getInt("Company_Id"));
					ps_addDetails.setTimestamp(14, rs_getDetails.getTimestamp("Complaint_Date"));
