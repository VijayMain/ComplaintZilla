...
public class Register_Complaint_DAO {
	...
	public boolean saveToDatabase(Register_VO beans, UserVO userVO, HttpSession session) {
		...
		try {
			...
			ResultSet rs_defecttype = st_defect.executeQuery("select * from Defect_Tbl");
			...
			if (flag1 == false) {
				PreparedStatement st1 = con.prepareStatement("insert into Defect_Tbl (Defect_Type) values(?)");
				st1.setString(1, str1);
				st1.executeUpdate();
			}

			PreparedStatement st_def_id = con
					.prepareStatement("select defect_id from Defect_Tbl where defect_type='" + beans.getDefect() + "'");
			...
			ps = con.prepareStatement(
					"insert into complaint_tbl(Complaint_No,U_Id,Cust_Id,Item_id,Defect_Id,Complaint_Received_by,"
			...
			if (beans.getUnregistered() != 0) {
				...
				PreparedStatement ps_unreg1 = con.prepareStatement(
						"update complaint_tbl_unassigned set Enable_Id=0 where unassign_id=" + beans.getUnregistered());
	...
	public String getCust_Name(int cid) {
		...
		try {
			...
			con = Connection_Utility.getConnection();
			ps = con.prepareStatement("select * from customer_tbl where Cust_Id='" + cid + "'");
		...
		try {
			...
			if (flag == true && bean.getUnregistered() != 0) {
				...
				while (rs_unreg.next()) {
					...
					PreparedStatement ps_unregatt = con.prepareStatement(
							"insert into complaint_tbl_attachments(Complaint_No,Attachment,Attach_Date,U_Id,File_Name,delete_Status)values(?,?,?,?,?,?)");
					ps_unregatt.setString(1, comp_no);
					ps_unregatt.setBlob(2, rs_unreg.getBlob("Mail_Attachment"));
					ps_unregatt.setTimestamp(3, rs_unreg.getTimestamp("Recieved_Date"));
					...
					PreparedStatement ps_unregHis = con.prepareStatement(
							"insert into complaint_tbl_attachment_hist(CA_Hist_date,Complaint_No,Attach_Date,U_Id,File_Name,Delete_Status_Hist)values(?,?,?,?,?,?) ");
					ps_unregHis.setTimestamp(1, bean.getDate());
					ps_unregHis.setString(2, comp_no);
					ps_unregHis.setTimestamp(3, rs_unreg.getTimestamp("Recieved_Date"));
				...
				PreparedStatement ps_unreg1 = con.prepareStatement(
						"update complaint_tbl_unassigned_attachment set Delete_Status=1 where unassign_id="
								+ bean.getUnregistered());
				ps_unreg1.executeUpdate();
